...
> [!TIP]
> See [CI.md](./CI.md) for a guide on integrating **augur** into GitHub Actions.
...
| `OPENAI_MODEL` | No | Model id; defaults to `gpt-5.4-nano` if unset. |
...
```bash
augur review local --base main --head HEAD --model gpt-5.4-nano --api-base https://api.example.com/v1 --api-key "$KEY"
```
